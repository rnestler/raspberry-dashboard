...
A Raspberry Pi dashboard application built with Rust and [Slint](https://slint.dev/) UI framework. Displays switchable widgets (clock, Snapcast now-playing) on a Raspberry Pi 3B+ connected to a screen. Automatically switches to the now-playing widget when a Snapcast stream is playing.
...
- **Main thread**: Runs the Slint event loop, owns all UI state. Slint timers handle clock updates (1s) and screensaver repositioning (5s).
- **Background thread**: Runs a tokio runtime for the async Snapcast client. Communicates UI updates to the main thread via `slint::invoke_from_event_loop()`.

### Widget system

`dashboard.slint` is the top-level window that conditionally renders widgets based on an integer `current-widget` property. TAB cycles widgets manually; the Snapcast module auto-switches based on playback state. To add a new widget: increment `WIDGET_COUNT` in `main.rs`, add a new `.slint` component, and add a conditional block in `dashboard.slint`.
