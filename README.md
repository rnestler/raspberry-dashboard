...
- `[remote_control]` — enables an HTTP server (`listen` address) so Home Assistant (or anything else) can switch widgets and blank/unblank the screen via `POST /widget/<name>` and `POST /blank/{on,off,toggle}`; requires `DASHBOARD_REMOTE_TOKEN` to be set (the section is ignored otherwise). See [config.toml.example](config.toml.example) for matching HA `rest_command` YAML
